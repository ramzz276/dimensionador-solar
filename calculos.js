...
function calcularPaso1_1() {
    ...
    const P_dispositivo = validarNumero(P_dispositivo_input, 'Potencia del dispositivo');
...
function calcularPaso1_2() {
    ...
    if (!estado.P_dispositivo) {
        mostrarError('Primero debes registrar la potencia del dispositivo (Paso 1.1)');
        return;
    }
...
function calcularPaso1_3() {
    ...
    const num_dispositivos = validarEntero(num_dispositivos_input, 'Número de dispositivos simultáneos');
    ...
    const contenido = `
        ...
        <div style="background: #e8f4f8; padding: 15px; border-radius: 8px; border-left: 4px solid #17a2b8; margin-top: 15px;">
            ...
            <p style="margin: 10px 0; font-size: 0.9em; color: #0c5460;">
                ...
                ${num_dispositivos} dispositivo(s) durante ${estado.t_sesion.toFixed(2)} hora(s).
            </p>
        </div>
    `;
...
function calcularPaso2_2() {
    ...
    if (!estado.E_autosuficiencia) {
        mostrarError('Primero debes calcular la energía requerida en la Sección 1 (Paso 1.3)');
        return;
    }
...
// PASO 2.3: Generar opciones de arreglo serie/paralelo
function calcularPaso2_3() {
    ...
    const V_nominal = validarNumero(V_nominal_input, 'Voltaje nominal del panel');
    if (!V_nominal) return;
...
function calcularPaso3() {
    ...
    if (!estado.paneles_confirmados || !estado.arreglo_seleccionado) {
        mostrarError('Primero debes seleccionar una configuración de paneles (Paso 2.3)');
        return;
    }
